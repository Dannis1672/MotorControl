...
- **HTTP REST API**：通过 `--http <port>` 参数启动 HTTP 服务器，所有控制操作可通过 REST API 调用
- **异步架构**：Modbus Worker 单线程处理写任务 + 周期性寄存器读取
...
连接 Modbus 设备、启动轮询线程并读取一次 PLC 寄存器快照记录到日志，**所有其他命令依赖此命令先执行**。
...
执行送粉检查流程：Z 轴下降 0.04 → 铺粉 → C 轴回零 → Z 轴回升 0.04，最后检查 PLC 铺粉到位信号。参数 `zd` 和 `fup` 为预留参数（当前硬编码为 0.04/-0.04）。返回 `true`/`false` 记录在日志中。
...
开始加工流程：打开激光电源和电机电源（设置 MW10 的 lase_power + motor_power 位）。

```
finish
```

结束加工流程：关闭激光、关闭使能、停止所有轴运动（清除 MW11 位 0-5）、关闭电机。
...
通过 `--http <port>` 命令行参数或 `config.json` 中的 `HttpPort` 字段启动 HTTP 服务器模式。

### 启动

```bash
# 命令行参数方式
./moto_control --http 8080

# 配置文件方式（设置 config.json 中 HttpPort 为非零值）
./moto_control
```

### API 端点

所有响应均为 JSON 格式（`Content-Type: application/json`），成功时 `result` 为 `"success"`，失败时为 `"failure"` 或返回 `{"error": "..."}`。除 `/api/state` 外，所有请求参数会以 info 级别记录到日志。
...
无请求体。返回 `{"result":"success"}` 或 `{"result":"failure"}`。
...
```
POST /api/bit
{"button": "Light", "value": 1}
```

| 字段 | 类型 | 说明 |
|------|------|------|
| `button` | string | 按钮名称，见 [CLI 按钮表](#数字量输出writebit) |
| `value` | int | 0 = 关，非 0 = 开 |
...
```
POST /api/float
{"param": "OxygenRatio", "value": 0.25}
```

| 字段 | 类型 | 说明 |
|------|------|------|
| `param` | string | 参数名称，见 [CLI 参数表](#浮点参数设定writefloat) |
| `value` | float | 设定值 |
...
返回完整系统状态 JSON，字段说明见 [状态字段参考](#状态字段参考)。
...
返回 `{"complete": true}` 或 `{"complete": false}`（注：`IsFeed` 会执行实际的轴运动操作，非纯查询）。
...
curl -X POST http://localhost:8080/api/move \
  ...
  -d '{"axis":"Z","distance":10.5}'

# 开灯
curl -X POST http://localhost:8080/api/bit \
  -H "Content-Type: application/json" \
  -d '{"button":"Light","value":1}'

# 设置氧含量
curl -X POST http://localhost:8080/api/float \
  -H "Content-Type: application/json" \
  -d '{"param":"OxygenRatio","value":0.25}'
...
> **注意**：以下按钮在 `WriteBit` 中未实现（调用无效果）：`BackFlush`、`CharmberPressureTest`、`WholePressureTest`、`ProcessMode`、`LaserCooler`、`PowderFeed`、`Feed`。

**UIFloat（浮点参数，10 个枚举；其中 8 个通过 HTTP API 可用，2 个仅 CLI/C++ SDK）：**
...
| `MotionControl::OverallLayers` | 总层数（⚠ 仅 CLI/C++ SDK，HTTP API 不支持） |
| `MotionControl::CurrnetLayer` | 当前层数（⚠ 仅 CLI/C++ SDK，HTTP API 不支持） |
...
| `MW12.*` | 位域对象 | 激光器/风机报警 |
| `MW12_12_14.*` | 位域对象 | 各轴驱动报警、运动超时报警、正/负限位、运动结束信号、铺粉到位 |
| `MW13_MW14.*` | 位域对象 | 阀门状态、急停、门锁、设备上电、原点标志 |
| `STOP.*` | 位域对象 | 各轴停止信号 |
| `Z/F/C轴加速度` | int | 对应轴的加速度设定值（PLC 寄存器 int32，单位=工程值×电子齿轮比） |
| `Z/F/C轴运动速度` | int | 对应轴的速度设定值 |
| `Z/F/C轴移动距离` | int | 对应轴的移动距离设定值（命令值） |
...
> 以下字段在 `config.json` 中存在但代码中未实际读取（超时硬编码为 100 ms）：
> `byte_timeout_us`、`response_timeout_us`
...
| 字段 | 说明 |
|---|---|
| `ModbusRetryDelayMs` | Modbus 读写失败后重试等待间隔（毫秒），默认 100 |
| `ReadInterval` | 周期性寄存器读取间隔（毫秒），默认 200 |
| `HttpPort` | HTTP 服务器端口，0 表示禁用 HTTP 模式（默认 0） |
| `Long_delay` | 长时间延迟（毫秒） |
| `First_time_delay` | 首次延迟（毫秒） |
| `Normal_delay` | 常规延迟（毫秒） |
...
> **重要**：MSVC 编译器不在默认 PATH 中，编译前需激活 Developer Command Prompt 环境。

#### 激活编译环境

打开 **Developer Command Prompt for VS 2022**（开始菜单搜索），或从普通 PowerShell 运行：

```powershell
cmd /c "call \`"C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars64.bat\`" > nul && powershell"
```
...
```powershell
where cl.exe
# 输出: C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Tools\MSVC\...\bin\Hostx64\x64\cl.exe
```

#### 编译命令

**方式一：Ninja 生成器（推荐，编译更快）**

```powershell
# 配置（首次需下载 spdlog、GoogleTest）
cmake -B build -G Ninja -DCMAKE_BUILD_TYPE=Debug

# 编译主程序
cmake --build build --target moto_control

# 编译并运行测试（26 个用例）
cmake --build build --target moto_control_test
.\build\moto_control_test.exe
...
产物位置：
- `build\moto_control.exe`
- `build\moto_control_test.exe`

**方式二：MSBuild 生成器（Visual Studio 集成）**
...
# 编译测试
cmake --build build --config Debug --target moto_control_test
.\build\Debug\moto_control_test.exe
```
...
| `-DBUILD_TESTING=ON` | ON | 编译 GoogleTest 单元测试 |
| `-G Ninja` | — | Ninja 生成器（推荐） |
| `-G "Visual Studio 17 2022"` | — | MSBuild 生成器 |

配置文件 `config.json` 会自动复制到输出目录。运行前确认串口号等参数与实际硬件匹配。
...
HTTP 服务器默认监听 `0.0.0.0`（所有网络接口）。如需限制访问，可使用防火墙规则：
...
3. **线程模型**：`ChuangRui_Control` 通过 `ModbusClient` 维护单个 Worker 线程处理写任务，同时每完成一个写任务后检查周期性寄存器读取间隔，无需额外的轮询线程。`ControlInitial` 配置并启动 Worker，`ControlFree` 停止。
