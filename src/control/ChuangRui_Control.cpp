...
Result ChuangRui_Control::AxisMove(Axis axis, float distance,bool wait) {//单轴运动
	Rs422 rs422;
	...
	ModbusTaskPush(rs422);
	if( wait == 1 ) return WaitForAxesMoveComplete(axis, distance);
...
Result ChuangRui_Control::AxisToZero(Axis axis) {	// 设置回原点指令位
	...
	*(uint16_t*)&out_data.MW11 |= (1 << axis);
	...
	// 等待回零完成
	int count = 0;
	while (count < global_config["Long_delay"].get<int>()) {
		count++;
		if (count == 1)
			this_thread::sleep_for(chrono::milliseconds(global_config["First_time_delay"].get<int>()));
		else
